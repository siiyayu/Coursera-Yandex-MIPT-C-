...
void TestFraction(int numerator, int denominator, int true_numerator, int true_denominator) {
    Rational r(numerator, denominator);
    AssertEqual(r.Numerator(), true_numerator , "Class numerator != " + true_numerator);
    AssertEqual(r.Denominator(), true_denominator, "Default class denominator != " + true_denominator);
}

void TestDefaultConstructor() {
    Rational r;
    AssertEqual(r.Numerator(), 0, "Default class numerator != 0");
    AssertEqual(r.Denominator(), 1, "Default class denominator != 0");
}

void TestConstructor() {
    TestFraction(4, 4, 1, 1);
    TestFraction(4, 2, 2, 1);
    TestFraction(2, 4, 1, 2);
    TestFraction(-4, 3, -4, 3);
    TestFraction(5, -3, -5, 3);
    TestFraction(-6, 3, -2, 1);
    TestFraction(16, -4, -4, 1);
    TestFraction(-10, -10, 1, 1);
    TestFraction(0, 10, 0, 1);
    TestFraction(0, -40, 0, 1);
}
