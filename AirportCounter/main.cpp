...
public:
    ...
    Items GetItems() const {
        ...
        for (const auto&  item : data) {
            result[i] = {static_cast<TAirport>(i), item};
            i++;
        }
        return result;
    }
...
void TestMostPopularAirport() {
    ...
    ASSERT(all_of(begin(most_popular), end(most_popular), [&](SmallCountryAirports a) { //Checks if
        // unary predicate p returns true for all elements in the range [first, last).
