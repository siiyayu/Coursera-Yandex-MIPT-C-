...
public:
    ReadingManager()
            : users_to_pages(),
              page_rating(MAX_PAGE_COUNT_ + 1, 0) {}

    void Read(int user_id, int page_count) {
        if (users_to_pages.count(user_id) == 0) {
            AddUser(user_id, page_count);
        }
        int prev_page_count = users_to_pages[user_id];
        users_to_pages[user_id] = page_count;
        page_rating[prev_page_count]--;
    ...
    double Cheer(int user_id) const {
        ...
        int page_count = users_to_pages.at(user_id);
        int sum = 0;
        int total = 0;

        for (int i = 0; i < page_rating.size(); i++) {
            if (i < page_count) {
                sum += page_rating[i];
            }
            total += page_rating[i];
        }
        auto res = (sum * 1.0) / (total - 1);
        return res;
...
//                position > 0
//                && page_count
//                   > user_page_counts_[sorted_users_[position - 1]]
//                ) {
...
//        while (
//                position < user_count
//                && user_page_counts_[sorted_users_[position]]
//                   == page_count
//                ) {
//            ++position;
//        }
...
//Все запросы Cheer в худшем случае работают O(Q^2): если в некоторый момент все добавленные пользователи имеют одинаковое количество прочитанных страниц, обработка Cheer для самого левого из них потребует линейного прохода.
...
//Суммарно все методы Read работают за UP + Q: сами вызовы требуют O(Q), а суммарное время работы UpdatePageRange не превосходит UP. Можно даже упростить до UP, поскольку по условию в методах Read номера страниц в рамках одного пользователя возрастают.
