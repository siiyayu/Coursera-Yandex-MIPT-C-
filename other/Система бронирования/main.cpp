...
//Суммарное время работы всех вызовов метода RemoveOldBookings. Поскольку каждое добавленное событие может быть удалено не более 1 раза, получаем не более Q вызовов метода PopBooking, то есть O(Q logQ).
//Итого, путём поглощения получаем итоговую оценку O(QL logQ). Величины T, C, R и W в оценке сложности не участвуют: T, R и C определяют лишь необходимую размерность типов, а W влияет лишь на количество удалений: чем больше W, тем меньше в конечном счёте будет вызовов PopBooking.
//
//1 / 1 балл